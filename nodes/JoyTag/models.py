...
class FastCLIPAttention2(nn.Module):
    ...
    def forward(
        ...
        q_states = q_states.view(bsz, tgt_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (bsz, num_heads, tgt_len, head_dim)
        k_states = k_states.view(bsz, src_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (bsz, num_heads, src_len, head_dim)
        v_states = v_states.view(bsz, src_len, self.num_heads, self.head_dim).transpose(
            1, 2
        )  # (bsz, num_heads, src_len, head_dim)
        ...
        with sdp_ctx:
            ...
            x = (
                x.transpose(1, 2).contiguous().view(bsz, tgt_len, embed_dim)
...
class FastCLIPEncoderLayer(nn.Module):
    def __init__(
        ...
        if skip_init is not None:
            self.attn_skip_init = SkipInit(
                hidden_size, channel_wise=True, init_scale=skip_init
            )
            self.mlp_skip_init = SkipInit(
                hidden_size, channel_wise=True, init_scale=skip_init
            )
        else:
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        if not self.use_palm_alt:
            hidden_states = self.self_attn(
                query_states=hidden_states, kv_states=hidden_states
            )
            ...
            if self.stochastic_depth is not None:
                attn = torchvision.ops.stochastic_depth(
                    attn, self.stochastic_depth, mode="row", training=self.training
                )
                mlp = torchvision.ops.stochastic_depth(
                    mlp, self.stochastic_depth, mode="row", training=self.training
                )
...
class CLIPEmbeddingLayer(nn.Module):
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if patch_dropout == seq_len or not self.training:
            ...
            embeddings = (
                patches.gather(
                    1, patch_mask.unsqueeze(-1).expand(-1, -1, self.hidden_size)
                )
            ...
            indices = torch.randperm(seq_len, device=pixel_values.device)[
                :patch_dropout
            ]
            embeddings = (
                patches[:, indices, :] + position_embeddings[indices.expand(1, -1)]
            )
...
class MHAPoolingHead(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor):
        hidden_states = self.layer_norm1(hidden_states)
        query_states = (
            self.probe.unsqueeze(0).unsqueeze(0).expand(hidden_states.size(0), 1, -1)
        )

        hidden_states = self.self_attn(
            query_states=query_states, kv_states=hidden_states
        )
...
class CLIPLikeModel(VisionModel):
    def __init__(
        ...
        stochastic_depth_space = (
            torch.linspace(0, stochastic_depth, num_layers)
            if stochastic_depth is not None
            else None
        )

        self.embedding_layer = CLIPEmbeddingLayer(
            embedding_dim,
            num_channels,
            image_size,
            patch_size,
        ...
        self.encoder_layers = nn.ModuleList(
            [
                FastCLIPEncoderLayer(
                    ...
                    skip_init=skip_init,
                    stochastic_depth=(
                        stochastic_depth_space[i].item()
                        if stochastic_depth_space is not None
                        else None
                    ),
                )
        ...
        if use_gap_head:
            ...
            self.pooling_head = MHAPoolingHead(
                embedding_dim,
                num_attention_heads,
                activation_cls,
                out_dim,
...
class MaskedAutoEncoderViT(nn.Module):
    def __init__(
        ...
        if not self.sine_positional_embeddings:
            ...
            self.decoder_positional_embeddings = nn.Embedding(
                self.seq_len, decoder_embedding_dim
            )
        ...
        self.patch_embedder = nn.Linear(
            num_channels * patch_size * patch_size, embedding_dim
        )
        ...
        if mae_post_norm:
            self.decoder_to_pixel_values = nn.Sequential(
                ...
                nn.Linear(
                    decoder_embedding_dim, num_channels * patch_size * patch_size
                ),
            )
        else:
            self.decoder_to_pixel_values = nn.Linear(
                decoder_embedding_dim, num_channels * patch_size * patch_size
            )
    ...
    def forward(self, batch):
        ...
        if self.sine_positional_embeddings:
            ...
            decoder_position_embeddings = sinusoidal_position_embedding(
                W // self.patch_size,
                H // self.patch_size,
            ...
            position_embeddings = self.positional_embeddings(self.position_ids)
            decoder_position_embeddings = self.decoder_positional_embeddings(
                self.position_ids
            )
    ...
    def calculate_loss(self, preds, batch, pos_weight):
        return (
            basic_calculate_loss(preds, batch, pos_weight, self.loss_type)
            + preds["mae_loss"]
        )
